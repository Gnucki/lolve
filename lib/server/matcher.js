...
Matcher.prototype.match = function(waiters) {
    ...
    for (var i = 0; i < waiters.length; i++) {
        ...
        if (waiter.for) {
            var waiterFor = waitingFor[waiter.for];

            if (null != waiterFor && waiterFor.for === waiter.player) {
                fights.push(
                    {
                        player1: waiterFor.player,
                        player2: waiter.player,
                        competitive: false
                    }
                );
            } else {
                waitingFor[waiter.player] = waiter;
            }
        } else {
            if (previousWaiter) {
                fights.push(
                    {
                        player1: previousWaiter.player,
                        player2: waiter.player,
                        competitive: true
                    }
                );
